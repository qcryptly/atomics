...
using matrix_states_t = typename cryptly::math::matrix_s<state_t, eigenvalue_t>::matrix_t;
...
TEST(wavefunction, jdown) {
	...
	for(auto i = 5.0; i > 0.0; i--) states.push_back(states[states.size() - 1].jdown());
	...
	angular_t angular_wf_expected_1{
		matrix_t {
			matrix_states_t{
				// First wf vector
				{
					{{{2.5,2.5}, {2.5,1.5}}, half_rt},{{{2.5,1.5}, {2.5,2.5}}, half_rt}
				},
				{
					{{{2.5,2.5}, {2.5,1.5}}, half_rt},{{{2.5,1.5}, {2.5,2.5}}, -1.0*half_rt}
				},
			},
		cryptly::math::srt_coupled<par_t>}, maximal};
