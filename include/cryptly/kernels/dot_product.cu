...
__global__ void dot_product_cuda(std::size_t N, const TArg *a, const TArg *b,
                                 TArg *r) {
...
int dot_product_impl<T, Ts...>::operator()(const kernel_ps_t &ps, std::size_t N,
                                           const T *a_i, const T *b_i, T *r_o) {
  ...
  dot_product_cuda<256, T>
      <<<ps.grid_size, ps.block_size, size_r, (CUstream_st *)ps.cuda_stream>>>(
          N, cuda_mem_a, cuda_mem_b, cuda_mem_r);
...
template <typename TArg, typename... Ts> void __hack_impl_dot_product__() {
  [[maybe_unused]] TArg *_ = nullptr;
  (dot_product{})(kernel_ps_t{}, std::size_t{}, _, _, _);
  __hack_impl_dot_product__<Ts...>();
}

template <> void __hack_impl_dot_product__<bool>() {
  [[maybe_unused]] bool *_ = nullptr;
  (dot_product{})(kernel_ps_t{}, std::size_t{}, _, _, _);
}

void __hack_dot_product__() { __hack_impl_dot_product__<primitive_types>(); }
