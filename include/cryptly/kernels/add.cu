...
__global__ void add_cuda(std::size_t n, TArg *a, TArg *b, TArg *r) {
...
int add_impl<T, Ts...>::operator()(const kernel_ps_t &ps, std::size_t N,
  ...
  add_cuda<T><<<ps.grid_size, ps.block_size, ps.shared_size,
                (CUstream_st *)ps.cuda_stream>>>(N, cuda_mem_a, cuda_mem_b,
                                                 cuda_mem_r);
...
template <typename TArg, typename... Ts> void __hack_impl__() {
  [[maybe_unused]] TArg *_ = nullptr;
  (add{})(kernel_ps_t{}, std::size_t{}, _, _, _);
  __hack_impl__<Ts...>();
}

template <> void __hack_impl__<bool>() {
  [[maybe_unused]] bool *_ = nullptr;
  (add{})(kernel_ps_t{}, std::size_t{}, _, _, _);
}

void __hack__() { __hack_impl__<primitive_types>(); }
